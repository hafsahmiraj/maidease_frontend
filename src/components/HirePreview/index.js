    ...
    const fetchData = async () => {
      try {
        ...
        if (hireResponse.ok) {
          ...
          const profileId =
            userType === "USER"
              ? hireData.maidHire.maid_id
              : hireData.maidHire.user_id;
          const profileUrl =
            userType === "USER"
          ...
          if (profileResponse.ok) {
            ...
            const ratingsResponse = await fetch(ratingUrl, {
              headers:
                userType !== "USER" ? { Authorization: `Bearer ${token}` } : {},
            });
  ...
  const handleStripePayment = async () => {
    try {
      ...
      const res = await fetch(
        "http://localhost:5000/api/maids/hire/stripe/create-session",
        {
          method: "POST",
          headers: {
        ...
        <div className="profile-grid">
          ...
          <div className="profile-right">
            ...
            >
              ...
              <TabPanel
                header={userType === "USER" ? "Maid Profile" : "User Profile"}
              >
                {userType === "USER" ? (
