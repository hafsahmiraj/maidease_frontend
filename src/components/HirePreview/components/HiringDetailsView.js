    ...
    return (
        <div className="hiring-details">
            ...
            <div className="profile-details">
                ...
                <p><strong>House Size:</strong> <span className="profile-value">{hiringDetails.house_size} Marla</span>
                </p>
                <p><strong>Number of Portions:</strong> <span
                    className="profile-value">{hiringDetails.no_of_portions}</span>
                </p>
                <p><strong>Number of People:</strong> <span
                    className="profile-value">{hiringDetails.no_of_peoples}</span></p>
                <p><strong>Hired Skills:</strong> <span
                    className="profile-value">{hiringDetails.hired_skills.join(", ")}</span></p>
                <p><strong>Preferred Work Time:</strong> <span
                    className="profile-value">{hiringDetails.preferred_work_time}</span></p>
                <p><strong>Total Amount:</strong> <span
                    className="profile-value">Rs. {hiringDetails.total_amount}</span></p>
                ...
                <p><strong>Created At:</strong> <span
                    className="profile-value">{new Date(hiringDetails.createdAt).toLocaleString()}</span></p>
                <p><strong>Updated At:</strong> <span
                    className="profile-value">{new Date(hiringDetails.updatedAt).toLocaleString()}</span></p>
            </div>
                    ...
                    <span className="profile-value">
                        ...
                        {!isUser && hiringDetails.acceptance_status === "Pending" && (
                            <Dropdown
                                ...
                                value={localDetails.acceptance_status}
                                onChange={(e) => {
                                    setLocalDetails(prev => ({ ...prev, acceptance_status: e.value }));
            ...
            {/* Wide Give Rating Button at the end */}
            {canShowRatings && (canGiveMaidRating || canGiveUserRating) && (
                <Button
                    label="Give Rating"
                    className="p-button-info wide-rating-btn"
                    style={{width: '100%', marginTop: 24}}
                    onClick={() => {
                        setShowRatingDialog(true);
                        setRatingTarget(canGiveMaidRating ? 'maid' : 'user');
                    }}
                />
            )}
            ...
            <Dialog
                ...
                footer={
                    <div>
                        ...
                        <Button label="Submit" onClick={handleSubmitRating} loading={ratingLoading}
